...
import HuanXiang.GongJu.DongTaiLiuTiXiaiHao;
import HuanXiang.GongJu.JiaoHuTuoDong;
import arc.Core;
...
public class KeNengYongDao {


    public static Vec2 照相机位置 = Core.camera.position;
    public static int 世界宽度 = Vars.world.tiles.width;
    public static int 世界高度 = Vars.world.tiles.height;
    public static boolean 世界生成 = Vars.world.isGenerating();
    ...
    public static boolean 规则1 = Vars.state.rules.editor && Vars.state.rules.infiniteResources;
    public static JiaoHuTuoDong 对话框拖动 = new JiaoHuTuoDong();
    ...
    public static <T extends Building> Consume 动态消耗流体(Func<T, LiquidStack[]> items) {
        return new DongTaiLiuTiXiaiHao(items);
    }
    ...
    public static Mods.LoadedMod 定位模组(String name) {
        return Vars.mods.locateMod(name);
    }

    public static boolean 有模组吗(String namesfad) {
        return Vars.mods.locateMod(namesfad) == null;
    }
    ...
    public static MappableContent 映射内容(ContentType a, String b) {
        return Vars.content.getByName(a, b);

    }

    public static Content 映射内容(ContentType a, int ID) {
        return Vars.content.getByID(a, ID);
