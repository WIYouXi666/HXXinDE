...
public class 游戏内数据方法 {
    ...
    public static void 设置标签(标签lib.标签 st, double se){
        设置数值(st.name+st.id, se);
    }
    public static void 标签增减(标签lib.标签 st, double se){
        数值增减(st.name+st.id, se);
    }
    public static boolean 标签数量比较(标签lib.标签 st, double se){
       return 获取数值(st.name+st.id)>se;
    }
    public static boolean 标签存在(标签lib.标签 st, double se){
        return 标签数量比较(st,0);
    }
    public static double 获取标签(标签lib.标签 st){
        return 获取数值(st.name+st.id);
    }
    public static void 删除标签(标签lib.标签 st){
        删除属性(st.name+st.id);
    }
