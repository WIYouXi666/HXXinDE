...
public class 分类栏 {
    ...
    boolean gridUpdate(InputHandler input){
        ...
        if(Core.input.keyTap(Binding.pick) && player.isBuilder() && !Core.scene.hasDialog()){ //mouse eyedropper select
            ...
            Block tryRecipe = build == null ? null : build instanceof ConstructBlock.ConstructBuild c ? c.current : build.block;
        ...
        if(Core.input.keyTap(Binding.block_info)){
            var build = world.buildWorld(Core.input.mouseWorld().x, Core.input.mouseWorld().y);
            Block hovering = build == null ? null : build instanceof ConstructBlock.ConstructBuild c ? c.current : build.block;
